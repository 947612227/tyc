...
#使用SQL查询数据的逻辑
# 入参有:table/dims/参数组 or SQL
# 出参有:main_id/gid
# 待完成
class Monitoring_data_spyder_by_SQL():
    # 获取main_id,gid的类
    def __init__(self, type,env):
        type = str(type)
        eventEndTime,eventStartTime=get_timestamp_and_previous_timestamp()
...
def send_monitor_message_by_main_id(table,main_id):
    url = f'http://{env.test_ip}/history/byTableAndRids?table={table}&rids={main_id}&syncType=2'
    print(f'url:{url}')
    r = requests.get(url)
    return f'{r.json()}'
...
if __name__ == '__main__':
    ...
    # r = send_monitor_message_by_main_id(table='company_license_merge',main_id='119822796')
    # print(r)
