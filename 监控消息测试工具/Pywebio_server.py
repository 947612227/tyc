...
import pandas as pd
from Monitor_singo_V1 import Monitoring_data_spyder, Data_conversion, singo_send_monitor_message
...
def task_1():
    ...
    if global_data['env'] == '线上':
        ...
        # obj = Monitoring_data_spyder(type=global_data['code'], env = test_ip)
    ...
    #0616临时修改为统一发测试
    if global_data['env'] == '线上':
        send_obj = singo_send_monitor_message(ip=test_ip)
    ...
    if entity_type == 'companyname':
        r = add_monitor_entity(username=username, password=password, group_name='测试分组1', cgids=[gids])
    elif entity_type=='organization':
        r = add_monitor_entity(username=username, password=password, group_name='测试分组1', brids=[gids])
    elif entity_type == 'human':
        r = add_monitor_entity(username=username, password=password, group_name='测试分组1', hcgid=[gids])


    put_code(r, 'json', 15)

    put_buttons(['发送消息'],[lambda:singo_send_monitor_message(ip=test_ip)])
