...
# data_list = read_csv('./csv/test_assume.csv')
data_list = read_csv('./csv/areacode.csv')

@pytest.mark.parametrize('data,',data_list)
def test_save_api(data,rich_data_path= 'csv/api_data/', json_file_path='test_api/'):
    # 读取接口数据
    no = data[0]  # 接口id
    url = data[1]  # 接口地址
    ...
    if csv_name == '':  # 无需替换参数时
        with open(f'{json_file_path}id_api.csv','a') as cf:
            wf = csv.writer(cf)
            wf.writerow([no,url,no])
    else:
        ...
        for data_dic in data_dic_list:
            url_re, json_name = replace_url(url, data_dic)
            with open(f'{json_file_path}id_api.csv', 'a') as cf:
                wf = csv.writer(cf)
                wf.writerow([no, url_re, f'{no}+{unquote(json_name)}'])
...
# def loop():
#     # data = read_csv('csv/test_assume.csv')
#     data = read_csv('test_api/id_api.csv')
